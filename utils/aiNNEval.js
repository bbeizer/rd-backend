 ...
 * same units as the search score (~ -1800 .. +1400), white-perspective; flipped
 * for black to match every other eval in the codebase.
 *
 * Feature order is dictated by mlp_weights.json#feature_keys: 4 planes × 64 cells
 * in a8→h1 order (pW, pB, bW, bB) followed by 3 scalar features. This exactly
 * matches extractAtomicFeatures in aiAtomicEval.js, so we reuse its scalar
 * helpers and skip the dict round-trip by writing directly into a Float64Array.
 */
...
const WEIGHTS_PATH = path.resolve(__dirname, '..', 'mlp_weights.json');
const raw = JSON.parse(fs.readFileSync(WEIGHTS_PATH, 'utf8'));

const FEATURE_KEYS = raw.feature_keys;
const N_IN = FEATURE_KEYS.length;          // 259
const N_H1 = raw.b1.length;                // 64
const N_H2 = raw.b2.length;                // 32
...
for (let i = 0; i < 64; i++) {
  if (FEATURE_KEYS[i] !== `pW_${ALL_CELLS[i]}`) {
    throw new Error(`mlp_weights.json feature order mismatch at ${i}: ${FEATURE_KEYS[i]}`);
  }
}

// Flatten weight matrices into Float64Arrays for fast inference.
...
const W1 = flatten(raw.W1, N_H1, N_IN);    // [64, 259]
const b1 = Float64Array.from(raw.b1);
const W2 = flatten(raw.W2, N_H2, N_H1);    // [32, 64]
const b2 = Float64Array.from(raw.b2);
const W3 = Float64Array.from(raw.W3[0]);   // [1, 32] → length 32
const b3 = raw.b3[0];

// Reused scratch buffers — eval is single-threaded, so this is safe and avoids
// allocating ~3KB per leaf evaluation in the search.
...
function evaluateNN(board, color) {
  ...
  // Canonicalize to "us-to-move": when it's black's turn, flip the board so the
  // featurizer always sees a white-to-move position. The NN was trained with
  // matching canonicalization, so output is sideToMove-positive — no post-hoc
  // negation. This enforces color-flip symmetry architecturally.
  const canon = color === 'white' ? board : flipAndSwapColors(board);
  buildInputVector(canon);
  return forward();
}

module.exports = { evaluateNN, FEATURE_KEYS };
