...
- **Phase C (in progress)**: NN eval inside alpha-beta — the Stockfish/NNUE pattern. `utils/aiNNEval.js` — MLP 259→64→32→1 with us-to-move canonicalization (color-flip symmetry enforced architecturally, post-2026-06-02). Currently 3/4 sanity pass (mirror diff still soft, learned-noise) and 0-2 vs B-Rabbit in dojo — NN's ceiling is its TD-Leaf teacher (B-Rabbit), so it can't surpass it on those labels. Next: train on outcome labels (`z ∈ {−1, 0, +1}`) from `data/selfplay/*.labeled.jsonl` to raise the ceiling above the teacher.
- **Phase D (next, high leverage)**: Scale the NNUE-style path. (1) Outcome-label retraining for a higher ceiling. (2) Bigger selfplay corpus (10K+ games — current 50 is undertrained). (3) Bigger network once data and labels justify it. (4) Speed/quantization — NN must be fast enough for deep alpha-beta search. This is incremental — same engine, better weights, dojo-gated each iteration. Stockfish reached world #1 on this exact path.
