...
- **B-Rabbit** (`hard`, also `impossible_brabbit` for benchmarks) — lean eval: 6 low-value features zeroed for speed, concave piece advancement (peaks at penultimate rank), win-points + coordination + flexibility features. Shipped as the new `hard` tier after combo_big NN took over `impossible`.
...
- **Phase D (explored, ceiling hit 2026-06-15)**: Scaled the NNUE-style path across 3 retraining rounds (4, 5, 6) — 200g combo_big-vs-combo_big, diverse 150g corpus, both 128→64 and 256→128 architectures. **All 6 candidates failed to beat combo_big** in head-to-head dojo (best result: round 6 XL 4-4 color-bound tie). Conclusions: (1) selfplay distribution collapses to 100% white wins under deterministic NN-vs-NN, degrading black-side training signal; (2) bigger architecture without proportionally more data underfits; (3) combo_big appears to have hit a near-local-optimum that incremental retraining doesn't escape. **Shipped combo_big as `impossible`, promoted B-Rabbit to `hard`** (2026-06-15). Path forward — instrumentation (Elo tracking, CI dojo gate) before further training rounds.
